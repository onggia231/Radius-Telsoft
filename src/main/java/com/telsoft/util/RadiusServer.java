...
import com.telsoft.packet.AccountingRequest;
...
public abstract class RadiusServer {
	...
	/**
	 * Constructs an answer for an RadiusPacket packet. This method
	 ...
	 *                exception is thrown, no answer will be sent
	 */
	public RadiusPacket requestReceived(RadiusPacket packet, InetSocketAddress client) throws RadiusException {
		RadiusPacket answer = new RadiusPacket(RadiusPacket.MESSAGE_RESPONSE, packet.getPacketIdentifier());
		copyProxyState(packet, answer);
		return answer;
	}
	...
	*/
	...
	protected void processRequest(final DatagramSocket s, final DatagramPacket packetIn) {
		try {
			...
			// parse packet: Đây là dữ liệu bên Client set attribute gửi sang
			final RadiusPacket request = makeRadiusPacket(packetIn, secret, RadiusPacket.UNDEFINED);
//			RadiusAttribute tranid = request.getAttribute(ParamContant.Transaction_Id);
//			String tranIdString = RadiusUtil.getStringFromUtf8(tranid.getAttributeData());
			if (logger.isInfoEnabled())
			...
			// Đây là response của Server trả về cho Client
	...
	protected RadiusPacket handlePacket(InetSocketAddress localAddress, InetSocketAddress remoteAddress, RadiusPacket request, String sharedSecret)
		...
		if (!isPacketDuplicate(request, remoteAddress)) {
			if (localAddress.getPort() == getAuthPort()) {
				...
				}else if(request instanceof HeartBeat){
					response = requestReceived(request, remoteAddress);
				}else if(request instanceof IPAccessContextRequest){
					response = requestReceived(request, remoteAddress);
				} else if(request instanceof IPAccessContextResponse){
					response = requestReceived(request, remoteAddress);
				}else if(request instanceof NewIPAccessContext){
					response = requestReceived(request, remoteAddress);
				}else if(request instanceof NewIPAccessContextAck){
					response = requestReceived(request, remoteAddress);
				}else if(request instanceof ReleaseIPAccessContext){
					response = requestReceived(request, remoteAddress);
				}else if(request instanceof ReleaseIPAccessContextAck){
					response = requestReceived(request, remoteAddress);
				}
				else
