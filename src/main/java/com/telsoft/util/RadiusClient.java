...
public class RadiusClient {
	...
	/**
	 * Sends an HearBeat,IPAccessContextRequest,IPAccessContextResponse,NewIPAccessContext... packet
	 * and receives a response packet.
	 *
	 * @param request request packet
	 * @return Radius response packet
	 * @throws RadiusException malformed packet
	 * @throws IOException     communication error (after getRetryCount() retries)
	 */

	public synchronized RadiusPacket commonProcess(RadiusPacket request)
			throws IOException, RadiusException {

		if (logger.isInfoEnabled()) {
			logger.info("send " + request.getClass().getSimpleName() + " packet: " + request);
		}
